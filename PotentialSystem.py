...
# Create the missing columns SkillPoints1, SkillPoints2, SkillPoints3, SkillPointsRookieRoll if they don't exist
for col_name in ['SkillPointsRookieRoll', 'SkillPoints1', 'SkillPoints2', 'SkillPoints3']:
...
def calculate_total_skill_points(row):
    # Calculate TotalSkillPoints by summing SkillPoints1 to SkillPoints3 and SkillPointsRookieRoll
    total_skill_points = row[['SkillPoints1', 'SkillPoints2', 'SkillPoints3', 'SkillPointsRookieRoll']].sum()
...
def calculate_age_based_skill_points(row):
    ...
    # Only assign values once for SkillPoints1, SkillPoints2, and SkillPoints3
    if (
        row['ContractStatus'] in ['Signed', 'FreeAgent', 'PracticeSquad']
        and row['Age'] in [20, 21, 22, 23, 24, 25]
        and row['Position'] not in ['QB', 'K', 'P']
    ):
        skill_points_columns = ['SkillPoints1', 'SkillPoints2', 'SkillPoints3']
...
# Calculate SkillPoints_Rookie by summing SkillPointsRookieRoll and random amounts from SkillPoints1 to SkillPoints3
...
def distribute_remaining_skill_points(row):
    ...
    if row['YearsPro'] == 'R':
        ...
        while True:
            ...
            # Ensure the sum of NewSkillPoints1 through NewSkillPoints3 equals RemainingSkillPoints
        ...
        row['NewSkillPoints1'], row['NewSkillPoints2'], row['NewSkillPoints3'] = new_skill_points
