...
# Your File Path
file_path = 'Files/Madden25/IE/Season10/AllProgRegInfo.xlsm'

# Season
season = 10 ###Change This###
...
df_players.to_csv('Files/Madden25/IE/Season10/PlayerTest.csv', sep=',',index=False)

# Excel Sheets Dataframe (Logic)
df_logic = pd.read_excel('Files/Madden25/IE/Season10/ProgRegLogicCheck.xlsx', sheet_name='Sheet1')
df_logic['StatHigh'] = df_logic['StatValue'].apply(make_high)
df_logic['StatLow'] = df_logic['StatValue'].apply(make_low)
df_logic.to_csv('Files/Madden25/IE/Season10/LogicTest.csv', sep=',',index=False)
...
# Filter Dataframes
df_offensiveStats = df_offensiveStats[
    (df_offensiveStats['SEAS_YEAR'] == season) &
    (df_offensiveStats['ContractStatus'] == 'Signed') &
    (
        ((df_offensiveStats['GAMESPLAYED'] >= 10) &
        (df_offensiveStats['DOWNSPLAYED'] >= 250) &
        ((df_offensiveStats['RECEIVECATCHES'] + df_offensiveStats['RUSHATTEMPTS'] >= 20))) |
...
& (df_kickingStats['GAMESPLAYED'] >= 6) & ((df_kickingStats['KICKFGATTEMPTS'] >= 15) | (df_kickingStats['PUNTATTEMPTS'] >= 30))]
df_returnStats = df_returnStats[(df_returnStats['SEAS_YEAR'] == season)
& (df_returnStats['KRETATTEMPTS'] + df_returnStats['PRETATTEMPTS'] >= 15)]
...
team_passing_ypg = df_offensiveStats.groupby('TeamIndex')['PASSYARDS'].transform('sum') / 17
...
df_offensiveStats.to_csv('Files/Madden25/IE/Season10/OffTest.csv', sep=',', index=False)
...
df_offensiveStats_unpivot = pd.melt(df_offensiveStats,id_vars=['FullName', 'Position', 'TeamName','RatingTier'],value_vars=['ScrimmageYardsPer1000DownsPlayed','TDsPer1000DownsPlayed','RBScrimmageYardsPer300Touches','RBScrimmageTDsPerGame','RUSHFUMBLES','WRPercentageofTeamPassYards' ],var_name='StatCheck',value_name='value')
...
df_off_points_agg.to_csv('Files/Madden25/IE/Season10/Points_off.csv', sep=',',index=False)
...
df_def_points_agg.to_csv('Files/Madden25/IE/Season10/Points_def.csv', sep=',',index=False)
...
df_oline_points_agg.to_csv('Files/Madden25/IE/Season10/Points_ol.csv', sep=',',index=False)
...
df_kickingStats_unpivot = pd.melt(df_kickingStats,id_vars=['FullName', 'Position', 'TeamName','RatingTier'],value_vars=['FGPercentage','EPPercentage','LongestFG','PuntTBPerIn20','YardsPerPunt','NetYardsToPuntYards'],var_name='StatCheck',value_name='value')
conn = sqlite3.connect(":memory:") # connect to Python memory to be able to query DataFrame variables as if they were tables
df_logic.to_sql("df_logic", conn, index=False)
...
df_kicking_points_agg.to_csv('Files/Madden25/IE/Season10/Points_kick.csv', sep=',',index=False)
...
#df_returnStats_unpivot = pd.melt(df_returnStats,id_vars=['FullName', 'Position', 'TeamName','RatingTier'],value_vars=['KRYardsPerReturn','PRYardsPerReturn','TDsPerReturn'],var_name='StatCheck',value_name='value')
#conn = sqlite3.connect(":memory:") # connect to Python memory to be able to query DataFrame variables as if they were tables
#df_logic.to_sql("df_logic", conn, index=False)
...
#df_return_points_agg = df_return_points.groupby(['FullName','Position','TeamName'])['SkillPointReturner'].sum().reset_index() # add all the skill points up
#df_return_points_agg.to_csv('Files/Madden25/IE/Season10/Points_return.csv', sep=',',index=False)
...
df_offensiveStats.to_csv('Files/Madden25/IE/Season10/OffTest.csv', sep=',',index=False)
df_defensiveStats.to_csv('Files/Madden25/IE/Season10/DefTest.csv', sep=',',index=False)
df_olineStats.to_csv('Files/Madden25/IE/Season10/OLTest.csv', sep=',',index=False)
df_kickingStats.to_csv('Files/Madden25/IE/Season10/KickingTest.csv', sep=',',index=False)
df_returnStats.to_csv('Files/Madden25/IE/Season10/ReturnTest.csv', sep=',',index=False)
# df_defensiveStats_unpivot.to_csv('Files/Madden25/IE/Season10/Defense_Unpivot.csv', sep=',',index=False)

# Export our Final Player DataFrame with updated skills points/regression points
df_final.to_csv('Files/Madden25/IE/Season10/Final_PreAdjustment.csv', sep=',',index=False)